...
We first read in the data tables for SI, we need to be in the Squid example directory in R:
...
```{r}
squid.subset <- select_vars(squid.data,c(25,11,21,6,19))
```
...
```{r, results='hide'}
Squid.subset.analysis <- run_MCMC(datas=squid.subset,nIter=30000,nBurnin=10000,nChains=3,nThin=30,Data.Type='Fatty.Acid.Profiles',Analysis.Type='Population.proportions',Rnot=2,plott=F,spawn=T)
...
```{r}
Pop.list <- list(Stable.Isotopes = Squid.SI.run1, Fatty.Acids = Squid.subset.analysis)
...
```{r, results='hide'}
Squid.subset.ind <- run_MCMC(datas=squid.subset,nIter=30000,nBurnin=10000,nChains=3,nThin=30,Data.Type='Fatty.Acid.Profiles',Analysis.Type='Individual.proportions',Rnot=5,plott=F,spawn=T)
...
```
Looking at the individual (still preliminary!) posterior of proportions, it seems that some of the uncertainty at the population level may stem from the dominance of Sheepshead minnow for squid 1,2 and 5, whereas Silverside dominantes for squid no 3 and 4. Nevertheless, a lot of uncertainty remains.
...
```{r, results='hide'}
Squid.combined.analysis <- run_MCMC(datas=squid.subset,nIter=30000,nBurnin=10000,nChains=3,nThin=30,Data.Type='Combined.Analysis',Analysis.Type='Population.proportions',plott=F,spawn=T)
...
#switched treatments
pred.treat <- c(grep('SF',rownames(pred.table.FA)),grep('SC',rownames(pred.table.FA)))
pred.treat[1:length(grep('SF',rownames(pred.table.FA)))] <- 1
pred.treat[(length(grep('SF',rownames(pred.table.FA)))+1):length(pred.treat)] <- 2

pred.table.FA <- pred.table.FA[c(grep('SF',rownames(pred.table.FA)),grep('SC',rownames(pred.table.FA))),]
# same here, replace
for (i in 1:ncol(pred.table.FA))
  pred.table.FA[pred.table.FA[,i]==0,i] = min(pred.table.FA[pred.table.FA[,i]>0,i])
...
```{r, results='hide'}
Squid.sc.analysis <- run_MCMC(datas=squid.subset,nIter=30000,nBurnin=10000,nChains=3,nThin=30,Data.Type='Fatty.Acid.Profiles',Analysis.Type='Population.proportions',Rnot=0.2,plott=F,spawn=T)
