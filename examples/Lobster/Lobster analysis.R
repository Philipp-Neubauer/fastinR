...
###########################################################
###### Data grooming:                    ##################
###### and a few (minor...or not) assumptions...  #########
###########################################################


# replace zero contributions by min /10 - more sensible choices can be made when the detection limit is known
for (i in 1:nrow(RLR.prey.means))
  RLR.prey.means[i,RLR.prey.means[i,]==0] <- min(RLR.prey.means[i,RLR.prey.means[i,]!=0])/100
# replace zeros in sds
for (i in 1:nrow(RLR.prey.sd))
  RLR.prey.sd[i,RLR.prey.sd[i,]<=1e-6] <- min(RLR.prey.sd[i,RLR.prey.sd[i,]>1e-6])/100

n.preys=ncol(RLR.prey.means)

#number of samples per prey group
nsamples = as.numeric(RLR[1,seq(3,ncol(RLR-1),2)])
...
dista <- adist(cbind(RLR.prey.means,RLR.means))

mds <- vegan::metaMDS(dista)

pl <- plot(mds,type='n')
points(pl,'sites',pch=21,col=1,bg=c(factor(group),8),cex=2)
legend('bottomleft',c(group,'Reserve Lobster'),pch=16,col=c(2:7,1))
points(pl,'sites',pch=21,col=1,bg=c(factor(group),8),cex=2)
...
dista <- adist(cbind(RLR.prey.means[six,],RLR.means[six]))

mds <- vegan::metaMDS(dista)

pl <- plot(mds,type='n')
points(pl,'sites',pch=21,col=1,bg=c(factor(group),8),cex=2)
legend('bottomleft',c(group,'Reserve Lobster'),pch=16,col=c(factor(group),8))
points(pl,'sites',pch=21,col=1,bg=c(factor(group),8),cex=2)
...
mprey <-  apply(RLR.prey.means[six,],2,clo)
...
for (i in 1:n.preys) RLR.sim[,,i] <- MASS::mvrnorm(ns,RLR.prey.means[six,i],diag(RLR.prey.sd[six,i]^2))
...
dista <- adist(cbind(t(apply(RLR.sim,2,I)),RLR.means[six]))

mds <- vegan::metaMDS(dista,trymax=100)

pl <- plot(mds,type='n')
points(pl,'sites',pch=21,col=1,bg=c(factor(rep(group,each=ns)),8),cex=1.2)
legend('bottomleft',c(group,'Lobster'),pch=16,col=c(factor(group),8))
points(pl,'sites',pch=21,col=1,bg=c(factor(rep(group,each=ns)),8),cex=1.2)
...
RLR.data <- list(datas.FA=datas.FA,n.preys=n.preys,n.preds=n.preds,even=even,prey.ix=group)

guiSet('datas',RLR.data)

# MCMC defaults to population proportions only from FA data,
RLR.org <- run_MCMC(nIter=100000,nBurnin=10000,nChains=3,nThin=100,datas = RLR.data,plott=F)

MCMCplot(RLR.org)
plot(RLR.org,save=F,density=T)
...
nsamples = tapply(nsample,group,sum)[idx]
