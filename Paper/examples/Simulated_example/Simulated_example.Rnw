...
separation and reduction in colinearity in the prey matrix. Using
...
to colinearity in the source matrix. We choose Fatty Acid no 3,2 and 6
...
The Rnot parameter sets the prior scale for the predator logistic normal covariance matrix, and will often determine how well the chain mixes, that is, how well it explores the parameter space, or if it gets stuck in local modes (that can be of low probability). In the latter case, one would see strong autocorrelation in the Markov Chains for individual paramters. Increasing \emph{Rnot} and/or \emph{Rnot\_{SI}} can help, but will make it harder to get precise estimates. There is thus a tradeoff between accuracy and mixing here, and often one will just have to run an analysis for many iterations and with a large thinning interval (e.g., >100k iterations with a thinning interval of >100). This should be done after finding reasonable parameters for Rnot on a shorter run, and then letting the final analysis run with long chains.
...
<<Plot Markov Chains and Densities>>=
MCMCplot(Pop.SI)
@
...
<<dignostics FAP 2nd try,eval=FALSE>>=
diags(Pop.FA2)
@
...
This section illustrates the model sensitivity to source separation and diet eveness. The package has simualted datasets for this illustration, names for their diet eveness (even vs uneven) and source separation (good vs bad). These can be loaded with:
...
To visualise, as above, 

<<plot sensitivity datasets 1>=
dataplot(even_good)
@
...
<<plot sensitivity datasets 2>=
dataplot(uneven_good)
@

Here, prey separation is good, but diets are skewed towards prey 1 \& 2. 

<<plot sensitivity datasets 3>=
dataplot(even_bad)
@
...
For all three examples, we'll use all 10 simualted FAs to make sure that the resutls are not biased by variable selection. We can run the MCMC directly:

select_vars(even_good)

<<MCMC for >>=
even.good.mcmc <- run_MCMC(datas = eg,
                    nIter = 10000,
                    nBurnin = 1000,
                    nChains = 3,
                    nThin = 10,
