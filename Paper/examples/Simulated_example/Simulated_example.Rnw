...
Mixing nly improves marginally, indicating that this may be good as it gets with stable isotopes alone.
...
The first simulation of idealised data shows that the model clearly recovers the variability and magnitude of population diet proportions. Even for uneven diet proportions, the model performs well, with posteriors reflecting simualted diets. For the last case with low resolution, the posteriors are wide and substantial uncertaitny thus reflects the inability to making clear statements about diet proportions.
