...
Start with estiamting SI discrimination based on fish and crustacean only diets. First we read the .csv files with raw data:
...
All done, checking output visually for adequacy (convergence, lack of strong autocorrelation in the Markov Chains) , extract estiamted coefficients for each preys species and isotope and put the resulting matrix into the right format.
...
It seems that despite the informative priors, the posterior for both beta.not[2] and beta.reg[2] (coeffs for $\Delta N$) are higher and lower, repsectively, in their posterior median than their respective priors. The high intercept leads to $\Delta N$ discrimination coefficients that are both high and similar in magnitude for both prey species, indicating that this may be linked to the tissue type or to the predator. For $\Delta C$ the posterior medians are close to the respective prior means. Estiamted $\Delta C$ discrimination is positive for both species.
