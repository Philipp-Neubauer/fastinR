...
This document details the estiamtion procedure for stable isotope dicrimination and fatty acid conversion coefficients. The data used is from Stowasser et al 2006 (JEMBE 333: 97–114), and is available as .csv files in the same folder as this document. We group all fish together as we don't know the proportions of individual fish species that were fed in the treatment. The uncertainty in estiamted dicromination coefficients will reflect any variability in species specific coefficients, so will mainly just introduce additional uncertainty in the final analysis.

NOTE: The fatty acid analysis takes a long time to run (possibbly several hours, depending on CPU speed and number of cores). Please factor this in when considering to repeat/modify this analysis.

\section*{Stable Isotope discimination}

We start with estiamting SI discrimination based on fish and crustacean only diets. First we read the .csv files with raw data:

\subsection*{Data grooming}

<<Read Data Tables,tidy=TRUE>>=
...
We now set up priors for the analysis. We need priors for both the intercept and concentration dependence of the priors. Priors for $\Delta N$ follow from Hussey et al 2014 (Ecology Letters Volume 17, Issue 2, pages 239–250, February 2014). Priors for $\Delta C$ are from Caut et al 2009 (Journal of Applied Ecology, 46, 443–453).
...
<<prior prey mean,tidy=TRUE,results='hide'>>=
require(dplyr)
...
All done, checking output visually for adequacy (convergence, lack of strong autocorrelation in the Markov Chains) and extract estiamted coefficients for each preys species and isotope and put the resulting matrix into the right format.
...
It seems that despite the informative priors, the posterior for both beta.not[2] and beta.reg[2] (coeffs for $\Delta N$) are higher and lower in their posterior median than their respective priors. The high intercept leads to $\Delta N$ discrimination coefficients that are both high and similar in magniturde for both prey species, indicating that this may be linked to the tissue type or to the predator. For $\Delta C$ the posterior medians are close to the respective priors means. Estiamted $\Delta C$ discrimination is positive for both species.
...
# use only samples taht were fed on single diet for >=20days
feed.type[which(regexpr('F',rownames(pred.table.FA))==1)[9:15]] <- 1
# take ouyt an early sample
...
<<write out>>=
# get estimated discrimintation from all chains:
...
#combine and write to file, repeat for 3 fish species in fianl analysis
