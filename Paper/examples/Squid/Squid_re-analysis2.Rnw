...
options(warn=-1)
library(fastinR)
#turn warnings off - they are distracting in the document

load("~/Work/FASTIN-R/Paper/examples/Squid/April15th.RData")
...
Squid.SC.analysis.alt <- run_MCMC(datas=squid.subset.treat.SC,
                              nIter=60000,
                              nBurnin=10000,
                              nChains=3,
                              nThin=20,
...
<<Plotting the diet proportions>>=
plot(Squid.SC.analysis.alt,save=F)
@

The posterior suggests a purely shrimp based diet. Does this suggest that FA turnover is much faster than the week-months time-frame commonly cited?
...
<<Combined NMDS, results='hide'>>=
dataplot(squid.subset.treat.SC.comb)
@

Curiously, separation seems to be worse, but this may be due to the NMDS not finding a good projection. Dimension reduction can be deceiving...
...
Squid.SC.comb.analysis.alt <- run_MCMC(datas=squid.subset.treat.SC.comb,
                                   nIter=60000,
                                   nBurnin=10000,
                                   nChains=3,
                                   nThin=20,
...
<<Marker Comparison,fig.height=4,fig.width=9>>=
...
Sicne we don't have overlapping data, it will be itneresting to see if the two markers can improve popualtion level estiamtes.
...
<<Combined SF proportions plot>>=
plot(Squid.SF.comb.analysis,save=F)
@
